<script setup >
import MailTemplateCard from "~/components/MailTemplateCard.vue";
...
const getTemplates = async () => {
  try {
    const response = await $api.get('/api/mails_template');
...
<template>
  <div class="mx-auto max-w-screen-xl flex gap-6">
    ...
    <div class="mt-4 w-full">
      <div>
        <h2 class="text-xl font-bold">Les templates</h2>
        <p class="text-gray-400 text-sm">Vous ne voulez pas créez vos propres mails ? Utilisez des templates !</p>
      </div>
      <div class="flex flex-wrap mt-4 gap-4 w-full" v-if="mails && mails.length !== 0">
        <MailTemplateCard v-for="mail in mails" :mail="mail" :key="mail.id"/>
      </div>
    </div>
  </div>
</template>
